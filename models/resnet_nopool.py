...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_channels=1, num_classes=10, width=1, base_width=16, withoutShortcut=False, withoutBN=False):
        ...
        self.output_layer = nn.Linear(128*16*block.expansion*width, num_classes)

        self.input_layer.base_fan_in   = 9*num_channels
        self.input_layer.base_fan_out  = 9*base_width
        self.output_layer.base_fan_in  = 8*base_width*block.expansion*16
...
def ResNet8NoPool(num_classes=10, num_channels=1, width=1, withoutShortcut = False, withoutBN=False):
    return ResNet(BasicBlock, [1,1,1,1], num_channels, num_classes, width, withoutShortcut=withoutShortcut, withoutBN=withoutBN)

def ResNet18NoPool(num_classes=10, num_channels=1, width=1, withoutShortcut = False, withoutBN=False):
    return ResNet(BasicBlock, [2,2,2,2], num_channels, num_classes, width, withoutShortcut=withoutShortcut, withoutBN=withoutBN)
