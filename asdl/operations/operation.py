...
OP_FOOF = 'foof'
OP_FOOF_INV = 'foof_inv'
OP_BOOB = 'boob'
OP_BOOB_INV = 'boob_inv'
...
ALL_OPS = [OP_FULL_COV, OP_FULL_CVP,
           ...
           OP_COV_KFE, 
           OP_FOOF, OP_FOOF_INV, OP_BOOB, OP_BOOB_INV]
...
class Operation:
    ...
    def forward_post_process(self, in_data: torch.Tensor, out_data: torch.Tensor):
        ...
        for op_name in op_names:
            ...
            elif op_name == OP_FOOF:
                A = self.cov_kron_A(module, in_data).mul_(cov_scale)
                self.accumulate_result(A, OP_FOOF, 'A')
    ...
    def backward_pre_process(self, out_grads, vector: torch.Tensor = None):
        ...
        for op_name in op_names:
            ...
            elif op_name == OP_FOOF_INV:
                A = self.cov_kron_A(module, in_data).mul_(cov_scale)
                del in_data
                A_inv = cholesky_inv(A, self._damping)
                del A
                self.accumulate_result(A_inv, OP_FOOF_INV, 'A_inv')
            elif op_name == OP_BOOB:
                B = self.cov_swift_kron_B(module, out_grads)
                self.accumulate_result(B, OP_BOOB, 'B')
...
class OperationContext:
    ...
    def cov_symmatrix(self, module, pop=False):
        ...
        cov_foof = self.get_result(module, OP_FOOF, pop=pop)
        if cov_foof is not None:
            kwargs['foof_A'] = cov_foof.pop('A', None)
            kwargs['foof_A_inv'] = cov_foof.pop('A_inv', None)
        cov_boob = self.get_result(module, OP_BOOB, pop=pop)
        if cov_boob is not None:
            kwargs['boob_B'] = cov_boob.pop('B', None)
            kwargs['boob_B_inv'] = cov_boob.pop('B_inv', None)
