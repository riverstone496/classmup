...
from .utils import cholesky_inv, smw_inv, normal_inv, low_rank_approximation, keep_diagonal
...
class SymMatrix:
    def __init__(self, data=None, inv=None, kron=None, kfe=None, diag=None, unit=None,
                 ...
                 foof=None, foof_A=None, foof_A_inv=None,
                 boob=None, boob_B=None, boob_B_inv=None,
        ...
        if not is_all_none([kron_A, kron_B, kron_A_inv, kron_B_inv]):
            ...
            self.kron: Kron = kron
        if not is_all_none([foof_A, foof_A_inv]):
            self.foof = Foof(foof_A, foof_A_inv)
        else:
            self.foof: Foof = foof
        if not is_all_none([boob_B, boob_B_inv]):
            self.boob = Boob(boob_B, boob_B_inv)
        else:
            self.boob: Boob = boob
    ...
    def __add__(self, other):
        ...
        values = {}
        for attr in ['data', 'kron', 'kfe', 'diag', 'unit', 'foof', 'boob']:
    ...
    def update_inv(self, damping=_default_damping, replace=False, A_Shape='Full', B_Shape='Full'):
        if self.has_data and not torch.all(self.data == 0):
            ...
            if replace:
                ...
                self.data = None
        if self.has_kron:
            self.kron.update_inv(damping, replace=replace, A_Shape='Full', B_Shape='Full')
        if self.has_diag:
            self.diag.update_inv(damping, replace=replace, A_Shape='Full', B_Shape='Full')
        if self.has_unit:
            self.unit.update_inv(damping, replace=replace, A_Shape='Full', B_Shape='Full')
...
class Kron:
    def __init__(self, A, B, A_inv=None, B_inv=None):
        ...
        self.damping_A,self.damping_B = None,None
    ...
    def update_inv(self, damping=_default_damping, calc_A_inv=True, calc_B_inv=True, eps=1e-7, replace=False, A_Shape='Full', B_Shape='Full'):
        if not self.has_data:
            raise ValueError('data do not exist.')
        ...
        if self.has_A and self.has_B:
            if len(self.A.shape)==0:
                ...
                A_Trace = self.A.trace()
            if len(self.A.shape)==0:
                B_Trace = self.B
            else:
                B_Trace = self.B.trace()
            A_eig_mean = (A_Trace if self.A_is_square else torch.sum(self.A ** 2)) / self.A_dim
            B_eig_mean = (B_Trace if self.B_is_square else torch.sum(self.B ** 2)) / self.B_dim
        ...
        if calc_A_inv:
            if A_Shape == 'Diag':
                self.A = keep_diagonal(self.A)
            if A_Shape == 'Top1':
                self.A = low_rank_approximation(self.A,1)
            if A_Shape == 'Top10':
                self.A = low_rank_approximation(self.A,10)
        if calc_B_inv:
            if B_Shape == 'Diag':
                self.B = keep_diagonal(self.B)
            if B_Shape == 'Top1':
                self.B = low_rank_approximation(self.B,1)
            if B_Shape == 'Top10':
                self.B = low_rank_approximation(self.B,10)
            ...
            if not torch.all(self.A == 0):
                if self.A_is_square:
                    try:
                        ...
                        self.A_inv = normal_inv(self.A, damping_A)
                else:
            ...
            if not torch.all(self.B == 0):
                if self.B_is_square:
                    try:
                        ...
                        self.B_inv = normal_inv(self.B, damping_B)
                else:
...
class Foof:
    def __init__(self, A, A_inv=None):
        self.A = A
        self.A_inv = A_inv
        self._A_dim  = None
        self.damping_A = None
    ...
    def eigenvalues(self):
        eig_A = symeig(self.A)
        return torch.sort(eig_A, descending=True)[0]

    def top_eigenvalue(self):
        eig_A = symeig(self.A)
        return (eig_A.max()).item()
    ...
    def update_inv(self, damping=_default_damping, calc_A_inv=True, eps=1e-7, replace=False, A_Shape='Full', B_Shape='Full'):
        self.damping_A = damping 
        if not self.has_data:
            raise ValueError('data do not exist.')
        if calc_A_inv:
            if not self.has_A:
            ...
            if not torch.all(self.A == 0):
                if self.A_is_square:
                    self.A_inv = cholesky_inv(self.A, damping)
                else:
                    self.A_inv = smw_inv(self.A, damping)
                if replace:
                    del self.A
                    self.A = None
    ...
    def mvp(self, vec_weight, vec_bias=None, use_inv=False, inplace=False):
        mat_A = self.A_inv if use_inv else self.A
        vec_weight_2d = vec_weight.view(-1, self.A_dim)
        mvp_w = vec_weight_2d.mm(mat_A).view_as(vec_weight)
        if inplace:
            vec_weight.copy_(mvp_w)
        if vec_bias is not None:
            return mvp_w, vec_bias
        return mvp_w

class Boob:
    def __init__(self, B, B_inv=None):
        self.B = B
        self.B_inv = B_inv
        self._B_dim  = None
        self.damping_B = None
    ...
    def eigenvalues(self):
        eig_B = symeig(self.B)
        return torch.sort(eig_B, descending=True)[0]

    def top_eigenvalue(self):
        eig_B = symeig(self.B)
        return (eig_B.max()).item()
    ...
    def save(self, root, relative_dir):
        ...
        for name in ['B']:
            ...
            tril_name = f'{name}_tril'
            relative_path = os.path.join(
                relative_dir, 'boob', f'{tril_name}.npy'
            )
            absolute_path = os.path.join(root, relative_path)
            _save_as_numpy(absolute_path, tril)
            relative_paths[tril_name] = relative_path
    ...
    def update_inv(self, damping=_default_damping, calc_B_inv=True, eps=1e-7, replace=False, A_Shape='Full', B_Shape='Full'):
        self.damping_B = damping 
        if not self.has_data:
            raise ValueError('data do not exist.')
        if calc_B_inv:
            if not self.has_B:
            ...
            if not torch.all(self.B == 0):
                if self.B_is_square:
                    self.B_inv = cholesky_inv(self.B, damping)
                else:
                    self.B_inv = smw_inv(self.B, damping)
                if replace:
                    del self.B
                    self.B = None
    ...
    def mvp(self, vec_weight, vec_bias=None, use_inv=False, inplace=False):
        mat_B = self.B_inv if use_inv else self.B
        vec_weight_2d = vec_weight.view(self.B_dim, -1)
        mvp_w = mat_B.mm(vec_weight_2d).view_as(vec_weight)
        if inplace:
            vec_weight.copy_(mvp_w)
        if vec_bias is not None:
            return mvp_w, vec_bias
