...
class GradientMaker:
    ...
    def backward(self,create_graph=False):
        self._loss.backward(create_graph=create_graph)

    def forward_and_backward(self,create_graph=False):
    ...
    def save_grad(self):        
        for name, module in self.module_dict.items():
            ...
            if module.weight.grad is not None:
                module.weight.prev_grad = module.weight.grad.detach().clone()
            if module.bias is not None and module.bias.grad is not None:
                module.bias.prev_grad = module.bias.grad.detach().clone()
    ...
    def grad_trans(self):
        ...
        for name, module in self.module_dict.items():
            if module.weight is None or module.weight.grad is None:
                continue
            if module.weight.prev_grad is None:
                continue
            vec=torch.flatten(module.weight.grad)
            prevec=torch.flatten(module.weight.prev_grad)
            if module.bias is not None and module.bias.grad is not None:
                vec = torch.cat([vec, torch.flatten(module.bias.grad)], dim=0)
                prevec = torch.cat([prevec, torch.flatten(module.bias.prev_grad)], dim=0)

            norm_dict[name]=float(torch.norm(vec)/torch.norm(prevec))
            cos_dict[name] = float(cos(vec,prevec))

        return cos_dict,norm_dict
