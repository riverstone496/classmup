...
class FisherMaker(GradientMaker):
    ...
    def _get_fvp_fn(self,damping=None):
        def fvp_fn(vec: ParamVector) -> ParamVector:
            self.forward_and_backward(fvp=True, vec=vec, damping=damping)
    ...
    def fisher_eig(self,
        ...
        eigvals, eigvecs = power_method(self._get_fvp_fn(damping=damping),
                                        self.model,
                                        top_n=top_n,
                                        max_iters=max_iters,
                                        tol=tol,
