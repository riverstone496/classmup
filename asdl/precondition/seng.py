...
class SengGradientMaker(PreconditionedGradientMaker):
    ...
    def vector_precond(self,module,vec,vec_bias=None,inv=False):
        data_size = self.config.data_size
        damping = self.config.damping
        info=self._curvature_info[module]
        bias = module.bias is not None and module.bias.requires_grad

        g = maybe_flatten_to_2d(vec)  # d_out x d_in
        if bias:
            g = torch.cat([g, vec_bias.unsqueeze(-1)], dim=1)  # d_out x (d_in + 1)
        # F = (dI + U'U)
        # F^{-1}g = g/d - U' @ (dI + UU')^{-1} @ Ug/d
        # g <- g/d
        g.div_(data_size).div_(damping)
        ...
        g.sub_(m)
        if bias:
            pre_G = g[:, :-1].contiguous().view_as(module.weight)  # d_out x d_in
            pre_Gias = g[:, -1].flatten()  # d_out
            pre_G = torch.cat([pre_G, pre_Gias.unsqueeze(-1)], dim=1)
        else:
            pre_G = (g.contiguous().view_as(module.weight))  # d_out x d_in
        return pre_G
