...
class KronPsgdGradientMaker(PsgdGradientMaker):
    ...
    def vector_precond(self,module,vec,vec_bias=None,inv=False):
        if vec_bias is not None:
            vec = torch.cat([vec, vec_bias.unsqueeze(-1)], dim=1)
        Ql,Qr = self.cholesky_factors[module]
        if inv:
            Ql = torch.inverse(Ql)
            Qr = torch.inverse(Qr)
        vec_precond = precond_grad_kron(Ql,Qr,vec)
        return vec_precond

    def fetch_cholesky(self,module):
        Ql,Qr = self.cholesky_factors[module]
        A=cholesky_inv(Ql,damping=1e-12)
        B=cholesky_inv(Qr,damping=1e-12)
        return A,B

    def fetch_eigen_range(self,module):
        Ql,Qr = self.cholesky_factors[module]
        LA = torch.linalg.eigvalsh(Ql)
        LB = torch.linalg.eigvalsh(Qr)
        min_eigen=1/float(torch.max(LA)*torch.max(LB))
        max_eigen=1/float(torch.min(LA)*torch.min(LB))
