...
def low_rank_approximation(matrix, rank):
    ...
    行列の低ランク近似を計算する関数
    Args:
        matrix(torch.Tensor): 入力行列
        rank(int): 近似行列のランク
    Returns:
        torch.Tensor: 低ランク近似された行列
    """
    # SVDを使用して、行列を分解する
    u, s, v = torch.svd(matrix)
    
    # 近似行列のランクに合わせて、分解した行列の各成分を切り捨てる
    u = u[:, :rank]
    s = s[:rank]
    v = v[:, :rank]
    
    # 低ランク近似された行列を構築する
    approx_matrix = torch.matmul(u, torch.matmul(torch.diag(s), v.t()))
    
    return approx_matrix

def keep_diagonal(matrix):
    """
    行列の対角成分以外を0にする関数
    Args:
        matrix(torch.Tensor): 入力行列
    Returns:
        torch.Tensor: 対角成分以外が0になった行列
    """
    # 入力行列の対角成分を取得する
    ...
    # 対角成分以外を0にした行列を作成する
    zero_matrix = torch.zeros(size).to(matrix.device)
    
    # 対角成分を新しい行列にコピーする
    diagonal_matrix = torch.diag(diagonal)
    
    # 対角成分以外を0にした行列と対角成分のみの行列を足し合わせることで、対角成分以外が0になった行列を作成する
    result = zero_matrix + diagonal_matrix
...
def normal_inv(X, damping=1e-7):
    if len(X.shape)==0:
        return torch.tensor([1/(X+damping)])
    diag = torch.diagonal(X)
    diag += damping
    u = torch.inverse(X)
    diag -= damping
