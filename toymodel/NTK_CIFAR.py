...
def filter_01(dataset):
    mask = (torch.tensor(dataset.targets) == 0) | (torch.tensor(dataset.targets) == 1)
...
class SimpleNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(32*32*3, 512)  # 32x32x3に変更
        self.fc2 = nn.Linear(512, 256)
        self.fc3 = nn.Linear(256, 2)  # クラス数を2に変更
...
def empirical_ntk_jacobian_contraction(fnet_single, params, x1, x2, block_size=64):
    # Compute J(x1)
...
# モデルを使用して予測
y_train_one_hot = F.one_hot(y_train, num_classes=2).float()  # クラス数を2に変更
