...
class ThreeLayerMLP(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, activation_type):
        ...
        self.fc3_2 = nn.Linear(hidden_size, output_size, bias=False)
        self.activation = nn.ReLU() if activation_type == 'ReLU' else nn.Identity()

    def forward(self, x, task_num = 0):
        """
...
class TransferTeacherModel(nn.Module):
    """
    ...
    def __init__(self, base_model, new_output_size):
        ...
        """
        super(TransferTeacherModel, self).__init__()
...
def fetch_h(model, inputs_for_dh, task_num = 0):
    model = register_fhook(model)
    inputs_for_dh = inputs_for_dh.to(device)
    y = model(inputs_for_dh, task_num)
    pre_act_dict = {}
    for name, module in model.named_modules():
        if len(list(module.children())) > 0:
              continue
...
def log_h_delta(epoch, model, init_pre_act_dict, tmp_pre_act_dict, inputs_for_dh, prefix = '', tmp_dh=True, task_num=0):
    pre_act_dict = fetch_h(model, inputs_for_dh, task_num=task_num)
    h_norm_dict = {}
    dh_norm_dict = {}
    int_dh_norm_dict = {}
    ...
    if tmp_dh:
        ...
        tmp_pre_act_dict = fetch_h(model, inputs_for_dh=inputs_for_dh, task_num=task_num)
    if args.wandb:
        wandb.log(log)
...
def prepare_log_h_delta(model, test_loader, task_num=0):
    for i, data in enumerate(test_loader, 0):
        inputs, labels = data
        break
    init_pre_act_dict = fetch_h(model, inputs,task_num)
    tmp_pre_act_dict  = fetch_h(model, inputs,task_num)
...
def initialize_optimizer(model, learning_rate, momentum, parametrization, width, muP_factor = 1e-3):
    ...
    if parametrization == 'muP':
        return optim.SGD([
            ...
            {'params': model.fc3_2.parameters(), 'lr': muP_factor * learning_rate / width}
        ...
        return optim.SGD([
            ...
            {'params': model.fc3_2.parameters(), 'lr': learning_rate / width}
        ], lr=learning_rate, momentum=momentum)
...
def train(model, optimizer, criterion, data_loader, test_data_loader, epochs, early_stopping_loss=None, prefix = '', task_num = 0):
    """
    ...
    inputs, init_pre_act_dict, tmp_pre_act_dict = prepare_log_h_delta(model, test_data_loader, task_num=task_num)
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0 or epoch == epochs - 1:
            test_loss, test_acc = test(model, criterion, test_data_loader, task_num=task_num)
            if task_num == 1:
                test_loss2, test_acc2 = test(model, criterion, task0_test_data_loader, task_num=0)
            tmp_pre_act_dict = log_h_delta(epoch, model, init_pre_act_dict, tmp_pre_act_dict, inputs_for_dh=inputs, prefix = prefix,task_num=task_num)
            ...
            if args.wandb:
                log = {
                    'epoch' : epoch+1,
                    prefix + 'Train_Loss_'+str(task_num) : average_loss,
                    prefix + 'Train_Acc_'+str(task_num) : accuracy,
                    prefix + 'Test_Loss_'+str(task_num) : test_loss,
                    prefix + 'Test_Acc_'+str(task_num) : test_acc,
                }
                if task_num == 1:
                    log[prefix + 'Test_Loss_0'] = test_loss2
                    log[prefix + 'Test_Loss_0'] = test_acc2
                wandb.log(log)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task2_lr', type=float, default=0.1, help='Learning rate for fine-tuning.')
    ...
    # Setup data loaders
    train_data_loader, task0_test_data_loader = setup_data_loaders(args.num_samples, args.input_size, args.output_size, teacher_model)

    # Determine loss function based on argument
    criterion = nn.CrossEntropyLoss() if args.loss_type == 'CE' else nn.MSELoss()

    # Train model
    model = train(model, optimizer, criterion, train_data_loader, task0_test_data_loader, args.epochs, args.train_loss_limit, 'Task1/')

    # Fine-tuning phase (if required)
    print("\nStart Task2\n")
    if args.task2_lr > 0:
        # Create Transfer model
        teacher_model = TransferTeacherModel(teacher_model,new_output_size=args.output_size)
        # Create Teacher Model
        teacher_model.fc1 = model_shift(teacher_model.fc1, args.input_shift_cor).to(device)
        teacher_model.fc3 = model_shift(teacher_model.fc3, args.output_shift_cor).to(device)
        ...
        model = train(model, finetune_optimizer, criterion, task1_data_loader, task1_test_data_loader, args.finetuning_epochs, args.train_loss_limit, 'Task2/', task_num=1)
